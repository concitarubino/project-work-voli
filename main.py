...
from modelli import Aeroporto, Volo, Scalo, Posto, Prenotazione, Bagaglio, Pagamento, Biglietto, Cliente
from schemi import AeroportoSchema, VoloSchema, ScaloSchema, PostoSchema, PrenotazioneSchema, BagaglioSchema, PagamentoSchema, BigliettoSchema, ClienteSchema
from crud import calcola_prezzo_prenotazione, gestisci_pagamento, calcola_prezzo_bagaglio, genera_codice_prenotazione, get_biglietti_cliente, get_dettagli_prenotazione, get_posti_disponibili, get_voli_da_a_a, get_voli_disponibili, crea_cliente
...
@app.get("/clienti", response_model=List[ClienteSchema])
def read_clienti(db: Session = Depends(get_db)):
    return db.query(Cliente).all()

@app.post("/clienti", response_model=ClienteSchema)
def create_cliente(cliente: ClienteSchema, db: Session = Depends(get_db)):
    esistente = db.query(Cliente).filter(Cliente.Email == cliente.Email).first()
    if esistente:
        raise HTTPException(status_code=400, detail="Email già registrata.")
    return crea_cliente(db, cliente)
